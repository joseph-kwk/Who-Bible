...
function main(){
  ...
  wikidata.forEach(item=>{
    ...
    if(best.score >= AUTO_THRESHOLD){
      ...
      high.push(item);
    } else if (best.score >= CANDIDATE_THRESHOLD){
  ...
  // Write outputs
  fs.writeFileSync(path.join('tools','people_from_wikidata.matched.relaxed.json'), JSON.stringify(high, null, 2), 'utf8');
  fs.writeFileSync(path.join('tools','people_from_wikidata.candidates.relaxed.json'), JSON.stringify(candidates, null, 2), 'utf8');
  fs.writeFileSync(path.join('tools','people_from_wikidata.unmatched.relaxed.json'), JSON.stringify(none, null, 2), 'utf8');
  ...
  fs.writeFileSync(path.join('tools','people_from_wikidata.link_report.relaxed.json'), JSON.stringify(summary, null, 2), 'utf8');
  console.log('Relaxed linking complete:', summary);
}
